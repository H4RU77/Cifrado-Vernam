...
public class Main {
    public static String encriptar (String frase,String clave){
        ...
        for (int bloque = 0; bloque<l; bloque++){
            ...
            numero = arrayXORInt[bloque];
            arrayXORChar[bloque] = (char)numero;
        ...
        for(int n:arrayXORInt){
            arrayXORChar[posicion]= (char)n;
            posicion++;
        }
        System.out.println(Arrays.toString(arrayXORChar));


        //Quitar comas, backets y espacios
        String formattedString = Arrays.toString(arrayXORChar)
                .replace(",", "")
                .replace("[", "")
                .replace("]", "")
                .replace(" ", "")
